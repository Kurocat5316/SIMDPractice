...
void simdifySceneContainers(Scene& scene)
{
	...
	else
	{
		...
		// allocate the correct amount of space at the correct alignment for SIMD operations
		scene.lightPosX = (__m256*) _aligned_malloc(sizeof(__m256) * scene.numLightsSIMD, 32);
		scene.lightPosY = (__m256*) _aligned_malloc(sizeof(__m256) * scene.numLightsSIMD, 32);
		scene.lightPosZ = (__m256*) _aligned_malloc(sizeof(__m256) * scene.numLightsSIMD, 32);
		
		scene.intensityR = (__m256*) _aligned_malloc(sizeof(__m256) * scene.numLightsSIMD, 32);
		scene.intensityG = (__m256*) _aligned_malloc(sizeof(__m256) * scene.numLightsSIMD, 32);
		scene.intensityB = (__m256*) _aligned_malloc(sizeof(__m256) * scene.numLightsSIMD, 32);
		...
		for (unsigned int i = 0; i < scene.numLightsSIMD * valuesPerVector; ++i)
		{
			...
			scene.lightPosX[i / valuesPerVector].m256_f32[i % valuesPerVector] = scene.lightContainer[sourceIndex].pos.x;
			scene.lightPosY[i / valuesPerVector].m256_f32[i % valuesPerVector] = scene.lightContainer[sourceIndex].pos.y;
			scene.lightPosZ[i / valuesPerVector].m256_f32[i % valuesPerVector] = scene.lightContainer[sourceIndex].pos.z;

			scene.intensityR[i / valuesPerVector].m256_f32[i % valuesPerVector] = scene.lightContainer[sourceIndex].intensity.red;
			scene.intensityG[i / valuesPerVector].m256_f32[i % valuesPerVector] = scene.lightContainer[sourceIndex].intensity.green;
			scene.intensityB[i / valuesPerVector].m256_f32[i % valuesPerVector] = scene.lightContainer[sourceIndex].intensity.blue;
		}
	}
}
